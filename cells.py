...
def get_nucleus_mask_brightfield(brightfield: np.ndarray) -> np.ndarray:
    tile_size = 512
    ...
    for r in range(0, num_rows, tile_size):
        for c in range(0, num_cols, tile_size):
            tile = brightfield[r:r+tile_size, c:c+tile_size, :]
            ...
            # Extract the hematoxylin channel (first channel after deconvolution)
            ...
            binary_mask_resized = cv2.resize(binary_mask.astype(np.uint8), (tile_size, tile_size))
            ...
            # Watershed transform (using OpenCV)
    ...
    center_tile_index_row = num_rows // 2 // tile_size
    center_tile_index_col = num_cols // 2 // tile_size

    plt.figure(figsize=(18, 6))

    plt.subplot(1, 2, 1)
    plt.imshow(binary_masks[center_tile_index_row * (num_cols // tile_size) + center_tile_index_col], cmap='gray')
    plt.title('Binary Mask (Center Tile)')
    plt.axis('off')

    plt.subplot(1, 2, 2)
    plt.imshow(segmented_nuclei[center_tile_index_row * (num_cols // tile_size) + center_tile_index_col], cmap='nipy_spectral')
    plt.title('Segmented Nuclei (Center Tile)')
    plt.axis('off')
...
def segment_nuclei_brightfield(brightfield: np.ndarray) -> np.ndarray:
def segment_nuclei_brightfield(brightfield: np.ndarray, window_size=512, visual_output=False) -> np.ndarray:
    ...
    for m in range(0, num_m_iterations):
        for n in range(0, num_n_iterations):
            ...
            local_mask = get_nucleus_mask_brightfield(box_center, codex, DAPI_index, window_size, erosion_radius, isolated=False, visual_output=False)
