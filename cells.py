...
class Nucleus:
    ...
    def __getitem__(self, item):
        if item > 2:
            raise IndexError(f'Tried to access spatial index {item} from nucleus: maximum spatial dimension is 2')
...
class Cell:
    ...
    def get_cell_mask_irregular(self, codex: np.ndarray, DAPI_index, cyto_index, opening_radius=2.5,
...
# If a small input sub-image is used, decrease the process count to minimize boundary artifacts (duplicate nuclei)
...
def extract_nuclei_coordinates_parallel(nuclei_mask: np.ndarray, num_processes: int, process_id: int, downsample_factor,
...
def segment_nuclei_dapi(codex: np.ndarray, dapi_index: int,
                        visual_output=False) -> np.ndarray:  # No return until nuclei extractor complete
...
def isolate_nuclei(nuclei_window: np.ndarray, opening_radius: float, visual_output=False) -> np.ndarray:
