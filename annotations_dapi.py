...
from cells import segment_nuclei_dapi, calculate_radii_from_nuclei, create_cells, extract_nuclei_coordinates, calculate_nuclei_sizes, slice_nucleus_window
from visualization import overlay_cell_boundaries, overlay_nuclei_boundaries, make_xml_annotations, overlay_nuclei_centers, generate_random_labels
...
if __name__ == '__main__':
    ...
    brightfield, codex = load_images(args, rotate_brightfield=False)
    ...
    nuclei_mask_brightfield = segment_nuclei_brightfield(brightfield)
    nuclei_brightfield = extract_nuclei_coordinates = extract_nuclei_coordinates(nuclei_mask_brightfield, downsample_factor=4,
                                                        num_processes=args.n_jobs, visual_output=False)
    ...
    if (len(cells) > 150000):
        logger.info('Downsampling cells...')
        cells = cells[::2]
    ...
    num_nuclei = len(clustered_nuclei)
    for m in range(0,num_nuclei, 35000):
        end_index = min(m+35000, num_nuclei)
        nuclei_subsample = clustered_nuclei[m:end_index]
        make_xml_annotations(tiff_section_names, nuclei_subsample, clustered_labels,m=m)
